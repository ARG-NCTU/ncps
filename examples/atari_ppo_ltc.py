...
class ConvLTCModel(RecurrentNetwork):
    ...
    def __init__(
        ...
        super(ConvLTCModel, self).__init__(
        ...
        # Preprocess observation with a hidden layer and send to LTC
        ...
        ltc_out, state_h = LTC(
            cell_size, return_sequences=True, return_state=True, name="ltc"
        )(
            inputs=dense1,
        ...
        # Postprocess LTC output with another hidden layer and compute values
        logits = tf.keras.layers.Dense(
            self.num_outputs, activation=tf.keras.activations.linear, name="logits"
        )(ltc_out)
        values = tf.keras.layers.Dense(1, activation=None, name="values")(ltc_out)
        ...
        if done and num_episodes is not None and num_episodes > 0:
            ...
            if record_gif:
                ...
                if training_hours is None:
                    imageio.mimsave(f'rl_gameplay/ltc/atari_gameplay_final_episode_{episode}.gif', frames, duration=33)
                else:
                    imageio.mimsave(f'rl_gameplay/ltc/atari_gameplay_training_time_{training_hours}_hours_episode_{episode}.gif', frames, duration=33)
...
ModelCatalog.register_custom_model("ltc", ConvLTCModel)
...
if __name__ == "__main__":
    ...
    os.makedirs(f"rl_ckpt/{args.env}/ltc", exist_ok=True)
    os.makedirs(f"rl_gameplay/ltc", exist_ok=True)
    if args.cont != "":
        algo.load_checkpoint(f"rl_ckpt/{args.env}/ltc/checkpoint-{args.cont}")
